...
extension CollectionVC {
    ...
    func tagEventsSinceLastLogin() {                            ///print("tagging blocks (saved timeblocks? \(savedTimeBlocksForProcessing)")
        ...
        if !indexPathsToProcess.isEmpty { // already (sort of) checked for this, in the caller (but used  event ArraysToProcess)
            
            let column = indexPathsToProcess.first![0]; let row = indexPathsToProcess.first![1]   ///; print("presenting popup at (\(column), \(row))")
            
            globalEventIdentifier = "\(taskArraysToProcess.first![eventIndex].eventDescription)"
